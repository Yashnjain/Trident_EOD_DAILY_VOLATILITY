...
def remove_existing_files(files_location):
    ...
    try:
        ...
        if len(files) > 0:
            ...
            logging.info("No existing files available to remove")
            print("No existing files available to remove")
    except Exception as e:
        logging.exception(e)
        raise e
def login_and_download(username, password, url, subject, download_path, logging):
    '''This function downloads log in to the website'''
    try:
        logging.info("Inside login and download function")
        ...
        try:
            ...
            logging.exception('driver not closed')
...
def extract_and_upload_pdf(download_path):
    ...
    try:
        logging.info("Inside extract_and_upload_pdf function")
        ...
        for file in file_name:
            if '.pdf' in file:            #checking the downloaded file is the recent one or not.Else, required file was not yet sent to mail.
                logging.info(f"Process started for file {file} from pdf")
                ...
                logging.info("Taking the areas of iso_name_1")
                ...
                logging.info("Taking the areas of iso_name_2")
                ...
                logging.info("Taking the areas of iso_name_3")
                ...
                logging.info("Taking the areas of iso_name_4")
                ...
                logging.info("DateFrame with all details got ready")
                print(main_df)
            else:
                print(f"Recent file with Trade_Date still not came")
                break
            rows += upload_in_sf(main_df, temp_date)
            logging.info("Upload sf function completed")
...
def upload_in_sf(df, trade_date):
    logging.info("Inside upload_in_sf function")
    total_rows = 0
    try:
        ...
        conn = engine.connect()
        logging.info("Engine object created successfully")
        ...
        logging.info("Check if the data is already present")
        sf_max_date =max_date[0][0].strftime("%Y-%m-%d")
        if trade_date > sf_max_date:
            if len(check_rows) == 0:
                logging.info(f"NO data of {trade_date} is present in {tablename} table.Dumping got started into table")
                ...
                logging.info(f"values of {trade_date} file are added into {tablename} table")
            elif len(check_rows)> 0:
                logging.info(f"data of {trade_date} file is already present in {tablename} table")
                print(f'Data already existed for {trade_date}')

            logging.info(f"Dataframe Inserted into the table {tablename} for TRADEDATE {trade_date} and total rows are {len(df)}")
            total_rows += len(df)
        elif trade_date <= sf_max_date:
            logging.info(f"New pdf file still not received.Last dumped data is of {sf_max_date} file")
            print("New pdf file not received")    
    except Exception as e:
        logging.exception("Exception while inserting data into snowflake")
        logging.exception(e)
        raise e
    finally:
        try:        
            conn.close()      
            engine.dispose()
            logging.info("Engine object disposed successfully and connection object closed")
            return total_rows
        except Exception as e:
            logging.exception(e)
            raise e
...
if __name__ == '__main__':
    try:
        ...
        log_file_location = os.getcwd() +"\\logs\\"+'TRIDENT_REC_PRICE_DATA.txt'
        ...
        logging.info("Calling remove existing file function")
        remove_existing_files(download_path)
        logging.info("Remove existing file completed")
        logging.info("Calling login download function")
        login_and_download(username, password, url, subject, download_path, logging)
        logging.info("Login and download function completed")
        logging.info("Calling extract df function")
        rows = extract_and_upload_pdf(download_path)
        logging.info("Extract df function completed")
